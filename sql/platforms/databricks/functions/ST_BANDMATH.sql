-- ST_BANDMATH for Databricks SQL Warehouse
-- Performs pixel-wise arithmetic between two bands
-- Operators: '+', '-', '*', '/'
...
CREATE OR REPLACE FUNCTION ${catalog}.${schema}.ST_BANDMATH(
    band1 BINARY,
    band2 BINARY,
    op STRING,
    metadata STRING,
...
LANGUAGE PYTHON
DETERMINISTIC
COMMENT 'Pixel-wise band arithmetic (+, -, *, /). Supports interleaved layout and JPEG/WebP. Returns JSON array.'
ENVIRONMENT (
...
if band1 is None or band2 is None or metadata is None or op is None:
    return None
...
if op == '+':
    ...
    with np.errstate(divide='ignore', invalid='ignore'):
        result = np.where(pixels2 != 0, pixels1 / pixels2, np.nan)
else:
    return None
