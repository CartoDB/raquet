-- ST_BANDMATH
-- Performs pixel-wise arithmetic operations between two bands
--
-- Operations:
--   '+' : band1 + band2 (addition)
--   '-' : band1 - band2 (subtraction)
--   '*' : band1 * band2 (multiplication)
--   '/' : band1 / band2 (division)
--
-- Parameters:
--   band1: BYTES - First band (or 'pixels' column for interleaved layout)
--   band2: BYTES - Second band (or same 'pixels' column for interleaved layout)
--   op: STRING - Operation ('+', '-', '*', '/')
...
--
-- Returns: ARRAY<FLOAT64> - Result of pixel-wise operation, NULL for nodata
--
...
--   SELECT `cartobq.raquet.ST_BANDMATH`(r.band_1, r.band_2, '-', m.metadata, 0, 1)
--
--   -- Ratio of bands (interleaved layout - same pixels column for both)
--   SELECT `cartobq.raquet.ST_BANDMATH`(r.pixels, r.pixels, '/', m.metadata, 0, 1)

CREATE OR REPLACE FUNCTION `cartobq.raquet.ST_BANDMATH`(
    band1 BYTES,
    band2 BYTES,
    op STRING,
    metadata STRING,
...
AS r"""
    if (band1 === null || band2 === null || metadata === null || op === null) {
        return null;
    }
    ...
    switch (op) {
        ...
        case '-': operate = (a, b) => a - b; break;
        case '*': operate = (a, b) => a * b; break;
        case '/': operate = (a, b) => b === 0 ? null : a / b; break;
        default: throw new Error(`Unknown operation: ${op}. Use '+', '-', '*', or '/'`);
    }
