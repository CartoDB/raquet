...
class TestRaquet2Geotiff (unittest.TestCase):
    def test_europe_parquet(self):
        ...
        self.assertEqual([round(n, 8) for n in geotiff_info["geoTransform"]], [0.0, 4891.96981025, 0.0, 10018754.17139462, 0.0, -4891.96981025])

        self.assertEqual([(b["block"], b["type"]) for b in geotiff_info["bands"]], [([256, 256], 'Byte'), ([256, 256], 'Byte'), ([256, 256], 'Byte'), ([256, 256], 'Byte')])
    ...
    def test_colored_parquet(self):
        ...
        self.assertEqual([colorEntry for colorEntry in band["colorTable"]["entries"][:6]], [[0, 0, 0, 0], [0, 255, 0, 255], [0, 0, 255, 255], [255, 255, 0, 255], [255, 165, 0, 255], [255, 0, 0, 255]])
